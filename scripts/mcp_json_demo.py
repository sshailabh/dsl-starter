#!/usr/bin/env python3
"""
JSON Parser demo using ANTLR4 MCP server.

Demonstrates parsing JSON and generating a parser.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="JSON Parser demo")
...
def main() -> int:
    ...
    grammar_path = repo_root / "dsl-starter" / "grammar" / "JsonSubset.g4"
    sample_path = repo_root / "dsl-starter" / "samples" / "data.json"
    ...
    sample_input = (
        ...
        else '{"name": "Alice", "age": 30, "tags": ["developer", "python"]}'
    )

    if args.server == "docker":
        cmd = ["docker", "run", "-i", "--rm", args.image]
    else:
        raise RuntimeError("Only docker mode supported in this demo")

    client = McpStdioClient.start(cmd)
    try:
        client.initialize()

        print("[1/3] Validating JSON grammar...")
        validation = client.call_tool("validate_grammar", {"grammar_text": grammar_text, "grammar_name": "JsonSubset"})
        if not validation.get("success"):
            print(json.dumps(validation, indent=2))
            return 1
        print("[ok] Grammar is valid")

        print("\n[2/3] Parsing JSON sample...")
        parsed = client.call_tool(
            "parse_sample",
            {"grammar_text": grammar_text, "sample_input": sample_input, "start_rule": "json", "show_tokens": False},
        )
        if parsed.get("success"):
            print(f"[ok] JSON parsed successfully")
            print(f"Parse tree preview: {parsed.get('parseTree', '')[:200]}...")
        else:
            print(f"[error] Parse failed: {parsed.get('errors', [])}")
            return 1
