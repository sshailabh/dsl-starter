#!/usr/bin/env python3
"""
Config DSL demo using ANTLR4 MCP server.

Demonstrates parsing configuration files and generating a parser.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Config DSL demo")
...
def main() -> int:
    ...
    grammar_path = repo_root / "dsl-starter" / "grammar" / "ConfigDsl.g4"
    sample_path = repo_root / "dsl-starter" / "samples" / "config.ini"
...
"""
    ...
    try:
        ...
        print("[1/3] Validating Config grammar...")
        validation = client.call_tool("validate_grammar", {"grammar_text": grammar_text, "grammar_name": "ConfigDsl"})
        if not validation.get("success"):
            print(json.dumps(validation, indent=2))
            return 1
        print("[ok] Grammar is valid")

        print("\n[2/3] Parsing config file...")
        parsed = client.call_tool(
            "parse_sample",
            {"grammar_text": grammar_text, "sample_input": sample_input, "start_rule": "config", "show_tokens": False},
        )
        if parsed.get("success"):
            print(f"[ok] Config parsed successfully")
            print(f"Parse tree preview: {parsed.get('parseTree', '')[:300]}...")
        else:
